...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (apiResponse.status !== 200) {
      ...
      let errorMsg = 'Claude API error ' + apiResponse.status;
      try {
        const errData = JSON.parse(apiResponse.body);
        if (errData.error && errData.error.message) errorMsg = errData.error.message;
        else if (errData.error) errorMsg = JSON.stringify(errData.error);
      } catch (_) {}
      return {
...
function corsHeaders(event) {
  ...
  const allowed = ['https://clauseai.net', 'https://www.clauseai.net', 'http://localhost:8767', 'http://127.0.0.1:8767'];
